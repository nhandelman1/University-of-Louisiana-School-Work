...
//Computes the coefficient matrices (C = MG) where M = hermite basis and//
...
void compCoeffMats()
{
	...
	{
		left = (i+(plane_1_path.num_points-1))%plane_1_path.num_points;
		end = (i+1)%plane_1_path.num_points;
		plane_1_path.tangents[i] = plane_1_path.points[left] - plane_1_path.points[end];
	}//end for

	for(int start = 0; start < plane_1_path.num_points; start++)
	{
		end = (start+1)%plane_1_path.num_points;
		
		//set the geometric matrix
		geometric_matrix.set(
	plane_1_path.points[start][0], plane_1_path.points[start][1], plane_1_path.points[start][2], 0,
	plane_1_path.points[end][0],   plane_1_path.points[end][1],   plane_1_path.points[end][2],   0,
	plane_1_path.tangents[start][0], plane_1_path.tangents[start][1], plane_1_path.tangents[start][2], 0,
	plane_1_path.tangents[end][0], plane_1_path.tangents[end][1], plane_1_path.tangents[end][2], 0);
	    
		//set the coefficient matrix
		plane_1_path.coeff_matrices[start] = hermite_basis*geometric_matrix;
	}//end for 

	//compute coefficient matrices for plane 2 cubic path segments
	for(int i = 0; i < plane_2_path.num_points; i++)
	{
		left = (i+(plane_2_path.num_points-1))%plane_2_path.num_points;
		end = (i+1)%plane_2_path.num_points;
		plane_2_path.tangents[i] = plane_2_path.points[left] - plane_2_path.points[end];
	}//end for

	for(int start = 0; start < plane_2_path.num_points; start++)
	{
		//array locations of nearest neighbors for each point indexed at i
		end = (start+1)%plane_1_path.num_points;

		//set the geometric matrix
		geometric_matrix.set(
	plane_2_path.points[start][0], plane_2_path.points[start][1], plane_2_path.points[start][2], 0,
	plane_2_path.points[end][0],   plane_2_path.points[end][1],   plane_2_path.points[end][2],   0,
	plane_1_path.tangents[start][0], plane_1_path.tangents[start][1], plane_1_path.tangents[start][2], 0,
	plane_1_path.tangents[end][0], plane_1_path.tangents[end][1], plane_1_path.tangents[end][2], 0);
	    
		//set the coefficient matrix
		plane_2_path.coeff_matrices[start] = hermite_basis*geometric_matrix;
	}//end for
}//end compCoeffMats
...
void drawPtsTans()
{
	...
	if(plane_1_path.show_pts_tans)//draw points and tangents for plane 1
	{
		for(int i = 0; i < plane_1_path.num_points; i++)
		{
			glPushMatrix();
	...
	if(plane_2_path.show_pts_tans) //draw points and tangents for plane 2
	{
		for(int i = 0; i < plane_2_path.num_points; i++)
		{
			glPushMatrix();
...
//																					 //
// Extra Commands:																	 //
//	1 - switch between left/right eye		    6 - switch show pts/tangents on/off	 //
